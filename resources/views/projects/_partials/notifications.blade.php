...
<div class="box">
    <div class="box-header">
        <div class="pull-right">
            <button type="button" class="btn btn-default" title="{{ Lang::get('notifications.create_hook') }}" data-toggle="modal" data-target="#notification"><span class="fa fa-plus"></span> {{ Lang::get('notifications.create_hook') }}</button>
        </div>
        <h3 class="box-title">{{ Lang::get('notifications.hook') }}</h3>
    </div>
    ...
    <div class="box-body table-responsive">
        <table class="table table-striped" id="notification_list">
            <thead>
                <tr>
                    <th>{{ Lang::get('notifications.name') }}</th>
                    <th>{{ Lang::get('notifications.webhook') }}</th>
                    <th>{{ Lang::get('notifications.notify_failure_only') }}</th>
                    <th>&nbsp;</th>
                </tr>
            </thead>
